...
import servo
from synDefQuick import *


class ProcessImage():

    def __init__(self):
        '''Initialize dynamic variables for video processing'''
        
        # variables for lead line
        self.contour_cond = 0
        self.contour_ok = 0
    ...
    def getCanny(self, image):
        '''get black and white image displaying contours'''
    ...
    def jitROI(image, delta, x0=0, y0=0, x1=0, y1=0):
        '''get Region Of Interest for a given image and dimensions. 
        Returns ROI image and the coo of the start - compiled version'''
        x0_ROI = x0 - delta
        x1_ROI = x1 + delta
        y0_ROI = y0 - delta
        y1_ROI = y1 + delta
    ...
    def showLeadLine(self, frame):
        '''show the the area the longest contour'''
    ...
    def flick(self):
         '''apply a sudden board movement if ball is stationary'''
    ...
    def dodgeHoles(holeContour, xball_cen, yball_cen, ball_coo):
        '''computes an anti-hole vector - compiled'''
    ...
    def getVector(contour, ball_coo, vectIter, nearest_idx_maze):
        '''Find the vector between the closest line position 
        and the current position of the object - compiled'''
    ...
    def getRollingWindow(self, frame):
        '''find the nodes along the lead line'''
    ...
    def windowGuts(xball_cen, yball_cen, nodes):
        '''finds nodes for the visibility window - compiled'''
        accuracyMode = 1
        distances = np.sqrt(
    ...
    def windowMasking(windowContour, windowFrame, xball_cen, yball_cen, accuracyMode):
        '''masks the area around the ball - compiled'''
    ...
    def processWindowImg(self, frame):
        ...
        if self.contour_ok:  
            ...
            # pass vector to the servo-controller
